window.setTimeout(function () {
  ...
  const explore = () => {
    ...
    if (navigator.userAgent.indexOf("Firefox") !== -1) span.style.top = "-5px";
    span.onclick = () => {
      root.textContent = '';
  ...
  const build = json => {
    ...
    /*  */
    json.slice(0, 6).forEach(({id, title}, index) => {
      if (id && title) {
        ...
        icon.textContent = title.replace(' -', '').replace(' &', '').split(' ').map(e => e[0]).slice(0, 2).join('').toUpperCase();
